...
export function getAvailableLoans(
  ...
  return LOANS.filter((l) => level >= l.minLevel && creditScore >= l.minCreditScore)
}

// ─── Calculations ───────────────────────────────────────────────

/** Calculate effective interest rate based on credit score */
export function calculateEffectiveRate(baseDef: LoanDef, creditScore: number): number {
...
export function calculateMaxLoanAmount(baseDef: LoanDef, creditScore: number): Decimal {
